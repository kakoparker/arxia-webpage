  ...
  return (
    ...
    >
      <div className="mx-auto max-w-[var(--content-max)] w-full flex items-center justify-between">
        ...
        {/* Mobile hamburger */}
        <button
          className="md:hidden text-gray-medium hover:text-white transition-colors duration-200 p-2 -mr-2"
      ...
      {/* Mobile overlay */}
      {menuOpen && (
        <div className="fixed inset-0 top-14 bg-blueprint-dark/98 backdrop-blur-[12px] md:hidden flex flex-col items-center justify-center gap-8 z-40">
          {navLinks.map((link) => {
            const isRoute = link.href.startsWith("/");
            const className =
              "font-[family-name:var(--font-jetbrains)] text-sm uppercase tracking-[2px] text-gray-medium hover:text-white transition-colors duration-200";
